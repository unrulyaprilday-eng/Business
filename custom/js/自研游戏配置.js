(function () {
  ...
  var games = [
    ...
    { code: "TP-3001", name: "Teen Patti", type: "Poker", isTeenPatti: true, rtp: 95.0, updated: "2026-08-24 10:00", capability: { roomTypes: ["低分房", "中分房", "高分房", "VIP房"], scoreUnit: "积分" }, rooms: [
      { id: 1, code: "TP-L", name: "低分房", baseScore: 10, minEntry: 100, rakeRate: 5, minRobotNum: 0, maxRobotNum: 3, minGold: 1000, maxGold: 100000, exitGameMinGold: 100, exitGameMaxGold: 200000, minPlayTime: 5, maxPlayTime: 120, minPlayRound: 1, maxPlayRound: 50 },
      { id: 2, code: "TP-M", name: "中分房", baseScore: 50, minEntry: 500, rakeRate: 5, minRobotNum: 0, maxRobotNum: 2, minGold: 5000, maxGold: 500000, exitGameMinGold: 500, exitGameMaxGold: 1000000, minPlayTime: 5, maxPlayTime: 180, minPlayRound: 1, maxPlayRound: 80 },
      { id: 3, code: "TP-H", name: "高分房", baseScore: 100, minEntry: 1000, rakeRate: 5, minRobotNum: 0, maxRobotNum: 1, minGold: 10000, maxGold: 1000000, exitGameMinGold: 1000, exitGameMaxGold: 2000000, minPlayTime: 10, maxPlayTime: 240, minPlayRound: 1, maxPlayRound: 100 },
      { id: 4, code: "TP-VIP", name: "VIP房", baseScore: 500, minEntry: 5000, rakeRate: 5, minRobotNum: 0, maxRobotNum: 1, minGold: 50000, maxGold: 2000000, exitGameMinGold: 5000, exitGameMaxGold: 5000000, minPlayTime: 10, maxPlayTime: 240, minPlayRound: 1, maxPlayRound: 120 }
    ], robotRules: {
      controlMode: "natural",
  ...
  function renderSummary(game) {
    ...
    if (game.type === "Poker") {
      var pokerSummary = FIXED_POKER_ROOM_COUNT + " 个房间 · 低分房底分 " + game.rooms[0].baseScore + " · 机器人 " + game.rooms[0].minRobotNum + "～" + game.rooms[0].maxRobotNum + " 人";
  ...
  function renderFields(game, module) {
    ...
    if (module === "pokerRooms") return [
      "<section class=\"self-game-config-section\"><h3>房间底分、最低进入条件与抽水比例</h3><p>房间类型来自游戏基础数据，SaaS 配置房间底分、最低进入条件和抽水比例；抽水比例默认 5%，范围 0%～100%，步长 0.1%。</p><table class=\"self-game-data-table self-game-room-table self-game-poker-room-config-table\"><thead><tr><th>ID</th><th>房间</th><th>房间底分</th><th>最低进入条件</th><th>抽水比例</th></tr></thead><tbody>",
      game.rooms.map(function (room) { return "<tr><td>" + room.id + "</td><td>" + room.name + "<span class=\"self-game-help\">（" + room.code + "）</span></td><td><input class=\"poker-base-score\" type=\"number\" min=\"0.1\" step=\"0.1\" value=\"" + room.baseScore + "\" data-room=\"" + room.code + "\"></td><td><input class=\"poker-min-entry\" type=\"number\" min=\"0.1\" step=\"0.1\" value=\"" + room.minEntry + "\" data-room=\"" + room.code + "\"></td><td><div class=\"self-game-input-wrap self-game-room-rate-input\"><input class=\"self-game-input poker-rake-rate\" type=\"number\" min=\"0\" max=\"100\" step=\"0.1\" value=\"" + room.rakeRate + "\" data-room=\"" + room.code + "\"><span class=\"self-game-input-suffix\">%</span></div></td></tr>"; }).join(""),
      "</tbody></table></section>"
    ].join("");
  ...
  function collectFields(root, game, module) {
    if (module === "robotRules") {
      ...
      game.rooms.forEach(function (room) { room.baseScore = Number(root.querySelector(".poker-base-score[data-room=\"" + room.code + "\"]").value); room.minEntry = Number(root.querySelector(".poker-min-entry[data-room=\"" + room.code + "\"]").value); room.rakeRate = Number(root.querySelector(".poker-rake-rate[data-room=\"" + room.code + "\"]").value); });
    } else if (module === "pokerRobots") {
      game.rooms.forEach(function (room) { ["minRobotNum", "maxRobotNum", "minGold", "maxGold", "exitGameMinGold", "exitGameMaxGold", "minPlayTime", "maxPlayTime", "minPlayRound", "maxPlayRound"].forEach(function (field) { room[field] = Number(root.querySelector("[data-field=\"" + field + "\"][data-room=\"" + room.code + "\"]").value); }); });
  ...
  function moduleSummary(game, module) {
    ...
    if (module === "pokerRooms") return game.rooms.map(function (room) { return room.name + " " + room.baseScore + "/" + room.minEntry + "/抽水" + room.rakeRate + "%"; }).join("；");
    if (module === "pokerRobots") return game.rooms.map(function (room) { return room.name + " 机器人" + room.minRobotNum + "-" + room.maxRobotNum + "人 · 金币" + room.minGold + "-" + room.maxGold; }).join("；");
