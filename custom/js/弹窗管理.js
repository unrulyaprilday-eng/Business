(function () {
  ...
  function renderTips(trigger) { var t = TIP[trigger] || TIP[T.reg]; tipP.textContent = t.p; tipS.textContent = t.s; }
  ...
  function openModal(item) {
    ...
    channelVisibilityEl.value = item && item.trigger === T.guest ? "\u4e0d\u9650\u5236" : "\u4ec5\u6e20\u9053\u53ef\u89c1";
    channelNamesEl.value = "";
    ...
    renderImageBlocks(); renderTips(formTriggerEl.value); renderChannelState(); renderButtonTextState(); renderConditions(formTriggerEl.value);
  ...
  document.getElementById("saveBtn").addEventListener("click", function () {
    ...
    var item = { id: state.editingId || String(Date.now()).slice(-9), name: document.getElementById("formName").value || "\u65b0\u5f39\u7a97\u914d\u7f6e", sortOrder: sortOrder, imageTheme: state.imageBlocks.length % 2 === 0 ? "light" : "purple", platform: getSelectedPlatforms().join(",") || "APP", trigger: formTriggerEl.value, jumpType: jumpMeta.type, jumpLink: jumpMeta.link, language: "\u8bed\u8a001", status: statusSwitchEl.classList.contains("on") ? T.on : T.off, startTime: document.getElementById("startTime").value, endTime: longTermEl.checked ? "2026-12-31 23:59:59" : endTimeEl.value, createdTime: "2026-06-30 14:30:00" };
  ...
  renderTable(state.rows); renderImageBlocks(); renderTips(formTriggerEl.value); renderChannelState(); renderButtonTextState(); renderConditions(formTriggerEl.value);
})();
